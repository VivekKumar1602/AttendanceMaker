...
if (isset($_SESSION['form_submitted']) && $_SESSION['form_submitted'] === true) {
    ...
    if (isset($_SESSION['last_submission_time']) && time() - $_SESSION['last_submission_time'] < 600) {
        ...
        header("Location: attMarker.php?error=$error");
        exit();
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($currCount < $maxLimit) {
        ...
        if ($event[$selectedRoll] === "P") {
            ...
            $sqlUpdate = "UPDATE tempattendancerecord 
            ...
            if ($conn->query($sqlUpdate) === TRUE) {
                ...
                header("Location: attMarker.php?message=$message");
                exit();
            } else {
                // Redirect back to the form page with an error message
