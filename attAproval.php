...
$sqlEvent = "SELECT * FROM tempattendancerecord ORDER BY id DESC LIMIT 1";
...
if (isset($_POST['approve'])) {
    if ($event) {
        ...
        $sqlCopy = "INSERT INTO attendancerecord (subject, date, ";
        
        for ($i = 1; $i <= 60; $i++) {
            $sqlCopy .= "`$i`, ";
        }
        
        $sqlCopy .= "`301`, `302`, `303`, `351`, `352`, `604`) SELECT subject, date, ";
        
        for ($i = 1; $i <= 60; $i++) {
            $sqlCopy .= "`$i`, ";
        }
        
        $sqlCopy .= "`301`, `302`, `303`, `351`, `352`, `604` FROM tempattendancerecord WHERE id = " . $event['id'];
        
        if ($conn->query($sqlCopy) === TRUE) {
            // Delete the row from attendanceRecord
            $sqlDelete = "DELETE FROM tempattendancerecord WHERE id = " . $event['id'];
...
if (isset($_POST['remove'])) {
    ...
    if ($event) {
        ...
        $sqlRemove = "UPDATE tempattendancerecord SET `$rollToRemove` = 'A', curr_students = curr_students - 1 WHERE id = " . $event['id'];
